...
int
seaf_util_rename (const char *oldpath, const char *newpath, gboolean bModifyAttribute)
{
#ifdef WIN32
	...
	if (bModifyAttribute)
	{
		dwAttr = GetFileAttributesW(newpathw);
		if (dwAttr == INVALID_FILE_ATTRIBUTES)
			dwAttr = FILE_ATTRIBUTE_NORMAL;
		else
			SetFileAttributesW(newpathw, dwAttr & ~FILE_ATTRIBUTE_READONLY);
	}
