                               ...
                               const char *email)
{
    ...
    *conflicted = FALSE;
    ...
    if (seaf_util_exists (file_path) &&
        seaf_util_rename (file_path, backup_path, TRUE) < 0) {
        ...
        *conflicted = TRUE;
        ...
        if (seaf_util_rename (tmp_path, conflict_path, TRUE) < 0) {
            goto bad;
        }
     ...
     */
    if (seaf_util_rename (tmp_path, file_path, TRUE) < 0) {
        seaf_warning ("Failed to rename %s to %s: %s. "
                      "Checkout server version as conflict file.\n",
        ...
        *conflicted = TRUE;
        ...
        if (seaf_util_rename (backup_path, file_path, TRUE) < 0) {
            seaf_warning ("Failed to rename %s to %s: %s. "
    ...
    if (force_conflict) {
        *conflicted = TRUE;
        ...
        if (seaf_util_exists (backup_path) &&
            seaf_util_rename (backup_path, conflict_path, TRUE) < 0) {
            seaf_warning ("Failed to rename %s to %s: %s. "
